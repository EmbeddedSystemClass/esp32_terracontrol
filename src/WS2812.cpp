...
const uint16_t chunkSize = 16 * 24;
uint16_t pos = 0;
...
WS2812::WS2812(gpio_num_t dinPin, uint16_t pixelCount, int channel) {
								...
								//ESP_ERROR_CHECK(rmt_set_tx_thr_intr_en(this->channel, true, -1));
...
void WS2812::show() {
								...
								for (uint16_t i = 0; i < this->pixelCount*24; i += chunkSize) {
																ESP_ERROR_CHECK(rmt_write_items(this->channel, this->items + i, chunkSize, 1));
								}

								//nextTick();
								// Show the pixels.
								//ESP_ERROR_CHECK(rmt_write_items(this->channel, this->items, this->pixelCount*24, 1));
} // show

void WS2812::nextTick() {
								if (pos < this->pixelCount*24) {
																ESP_ERROR_CHECK(rmt_write_items(this->channel, this->items + pos, chunkSize, 0));
																pos += chunkSize;
								}
}

const pixel_t& WS2812::getPixel(uint16_t index) const {
